# 🐛 Release v1.16.1 - Critical FTS5 Bug Fix
...
**Root Cause:**
```python
# ❌ This doesn't work as expected with FTS5!
cursor.execute("""
...
github_enescingoz_awesome_n8n_templates_ai product imagines|2  ← DUPLICATE!
```
...
|------|-----------|-----------|
| Search "product" | 4 FTS5 matches → 3 results | 3 FTS5 matches → 3 results ✅ |
| Search "AI" | Missing GitHub templates | ✅ Found GitHub templates |
| Search "image" | 4 FTS5 matches → 3 results | 3 FTS5 matches → 3 results ✅ |
| Category filter | Missing GitHub templates | ✅ Correct filtering |
| Direct FTS5 query | ✅ Worked (but with duplicates) | ✅ Works without duplicates |
...
**Key Learnings:**
1. FTS5 ≠ Regular Tables - Different conflict resolution behavior
...
*"Sometimes the best fixes are the ones that make you say 'Oh, of course!' after 6 rounds of testing."* 🐛→✅
